...
// picker position
const pickerX = 840, pickerY = 90;
const pickerW = 220, pickerH = 180;
...
function drawMirroredLine(x1, y1, x2, y2) {
  // offset from center
  let dx1 = x1 - drawCenterX, dy1 = y1 - drawCenterY;
  let dx2 = x2 - drawCenterX, dy2 = y2 - drawCenterY;
  ...
  if (symmetryMode === "none") {
    ...
    line(drawCenterX + dx1, drawCenterY + dy1, drawCenterX + dx2, drawCenterY + dy2);
    line(drawCenterX - dx1, drawCenterY + dy1, drawCenterX - dx2, drawCenterY + dy2);
    line(drawCenterX + dx1, drawCenterY - dy1, drawCenterX + dx2, drawCenterY - dy2);
    line(drawCenterX - dx1, drawCenterY - dy1, drawCenterX - dx2, drawCenterY - dy2);
  }
...
function mousePressed() {
  ...
  if (isClicked(buttonSymNone)) { symmetryMode = "none"}
  if (isClicked(buttonSymX))    { symmetryMode = "x"}
  if (isClicked(buttonSymXY))   { symmetryMode = "xy" }
  //clear
  ...
  for (let i = 0; i < swatches.length; i++) {
    ...
    if (mouseX > sx && mouseX < sx + swatchSize &&
        mouseY > 275 && mouseY < 275 + swatchSize) {
